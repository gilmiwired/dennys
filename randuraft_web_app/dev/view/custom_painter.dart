...
class TreePainter extends CustomPainter {
  ...
  TreePainter(this.nodeList,
      {this.offsetX = 100, this.offsetY = 100}); // オフセットを引数で受け取る
    ...
    nodeList.forEach((key, node) {
      ...
      final rect = Rect.fromPoints(
        Offset(node.x.toDouble() + offsetX,
            node.y.toDouble() + offsetY), // オフセットを加算
        Offset(node.x.toDouble() + nodeWidth + offsetX,
            node.y.toDouble() + nodeHeight + offsetY), // オフセットを加算
      );
      ...
      textPainter.paint(
          canvas,
          Offset(node.x.toDouble() + 5 + offsetX,
              node.y.toDouble() + 5 + offsetY)); // オフセットを加算
      ...
      for (String childID in node.children) {
        ...
        if (child != null) {
          canvas.drawLine(
            Offset(
                node.x.toDouble() + offsetX,
                node.y.toDouble() +
                    nodeHeight / 2 +
                    offsetY), // x座標を親ノードの左端に, y座標を中央に
            Offset(
                child.x.toDouble() + nodeWidth + offsetX,
                child.y.toDouble() +
                    nodeHeight / 2 +
                    offsetY), // x座標を子ノードの右端に, y座標を中央に
            Paint()
              ..color = Colors.red
              ..strokeWidth = 2,
          );
